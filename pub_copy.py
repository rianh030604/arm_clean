...
class ArmPubPoints(Node):
    ...
    def flip_j1(self):
        ...
        # tìm điểm đích thực tiếp theo
        next_real_idx = self.point_idx + 1
        while next_real_idx < len(POINTS) and POINTS[next_real_idx] is None:
            next_real_idx += 1

        if next_real_idx >= len(POINTS):
            self.get_logger().warn("No next target → skip flip")
            return

        target_pt = POINTS[next_real_idx]

        # vector hướng từ base → target
        x = target_pt['x']
        y = target_pt['y']
        ...
        self.get_logger().warn(
            f"FLIP theo target → atan2({y:.3f},{x:.3f}) = {desired_j1:.3f} rad"
        )

        # cập nhật IK result luôn
        if self.ik_result:
